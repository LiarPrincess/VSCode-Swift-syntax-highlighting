...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  const out = vscode.window.createOutputChannel("Swift+LANGUAGE_NAME", "Swift");

  out.appendLine(`Activating Swift+LANGUAGE_NAME`);
  out.appendLine(`Swift extension = vscode.getExtension(${SwiftExtension.id})`);
  const swiftExtension = vscode.extensions.getExtension(SwiftExtension.id);

  if (swiftExtension === undefined) {
    out.appendLine(`ERROR: Unable to find the official Swift extension.`);
    return;
  }

  if (!swiftExtension.isActive) {
    try {
      out.appendLine(`Swift extension -> activate()`);
      await swiftExtension.activate();
    } catch (error) {
      out.appendLine(`ERROR: ${JSON.stringify(error)}`);
      throw error;
    }
  }

  out.appendLine(`Swift extension -> getAPI()`);
  ...
  if (workspaceContext === undefined) {
    out.appendLine(`ERROR: Unable to obtain the 'workspaceContext' from the official Swift extension.`);
    return;
  }

  out.appendLine(`Swift extension -> override 'middleware.provideDocumentSemanticTokens'`);
